...
class VideoQRScanner:
    ...
    def process_video(self):
        ...
        # 继续处理下一帧
        threading.Timer(0.01, self.process_video).start()
...
class FileQRApp:
    ...
    def create_control_panel(self):
        ...
        # 扫描恢复按钮 - 添加下拉菜单
        scan_menu = tk.Menu(self.root, tearoff=0)
        scan_menu.add_command(label="从图片恢复", command=self.scan_restore)
        scan_menu.add_command(label="从视频恢复", command=self.scan_video)

        scan_btn = ttk.Button(btn_frame, text="扫描恢复", command=self.scan_restore)
        scan_btn.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        scan_btn.bind("<Button-1>", lambda e: scan_menu.post(e.widget.winfo_rootx(),
    ...
    def create_log_panel(self):
        ...
        # 添加欢迎消息
        self.add_log_entry("=== 文件二维码工具 v8.1 ===")
    ...
    def create_qr_codes(self, data, max_size=1800, version=8, mode="file", progress_callback=None):
        ...
        for i in range(total_chunks):
            ...
            if len(full_chunk) > max_size:
                ...
                return self.create_qr_codes(data, max_size, version, mode, progress_callback)
